...
public class MakeWhitePromotionMoveTest {
	...
	public void promotion() {
		test(fromFen("8/PPPPPPPP/8/4p3/8/7k/8/7K w - - 30 10"));
	}

	@Test
	public void promotionWithPossibleEnpassant() {
		test(fromFen("8/PPPPPPPP/8/4p3/8/7k/8/7K w - e6 30 10"));
	}
	...
	private void test(Game game) {
		...
		for(int i = 0; i < size; i++) {
			...
			case promotionRook:
				count++;
				test(move, game, promotionRook, WhiteRook);
				break;
			case promotionBishop:
				count++;
				test(move, game, promotionBishop, WhiteBishop);
				break;
			case promotionKnight:
				count++;
				test(move, game, promotionKnight, WhiteKnight);
				break;
			}
		}
